...
export function startIncident(cabin,{speed,sink,bank,airborne,fire,gear,cause}){
 cabin.phase=airborne?'descending':'evacuating';cabin.elapsed=0;cabin.fire=cabinClamp(fire);cabin.cause=cause;
...
export function tickCabin(cabin,dt,grounded){
 ...
 if(!grounded){cabin.phase='descending';return}cabin.phase='evacuating';cabin.elapsed+=dt;
 // Evacuation and fire exposure compete over a 20 second game sequence.
 const progress=cabinClamp(cabin.elapsed/20);
 for(const s of cabin.sections){const target=Math.min(s.survivors,Math.floor(s.occupants*progress*cabin.exits));s.evacuated=Math.max(s.evacuated,target)}
 if(cabin.elapsed>=20){for(const s of cabin.sections){const trapped=Math.max(0,s.survivors-s.evacuated),lost=Math.floor(trapped*cabin.fire*(.25+.6*s.damage));s.fatalities+=lost;s.survivors-=lost;s.injured=Math.min(s.survivors,s.injured+Math.floor((trapped-lost)*cabin.fire*.4));s.evacuated=s.survivors}cabin.phase='complete';cabin.final=true}
}
