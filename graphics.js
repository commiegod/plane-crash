...
diffuseColor*=mix(sampleA,sampleB,blendTex);
 ...
 function livery(type,scheme){const c=document.createElement('canvas');c.width=2048;c.height=512;const g=c.getContext('2d');g.fillStyle=scheme?.body||'#f4f5f5';g.fillRect(0,0,2048,512);g.fillStyle=scheme?.color||'#153e59';g.fillRect(0,0,2048,180);g.fillStyle=scheme?.accent||'#b49154';g.fillRect(0,180,2048,5);for(const y of [279,481])for(let x=220;x<1700;x+=31){g.fillStyle='#b3bdc0';g.beginPath();g.roundRect(x-1,y-1,15,13,4);g.fill();g.fillStyle='#203a47';g.beginPath();g.roundRect(x,y,13,11,3);g.fill()}g.strokeStyle='#75849055';g.lineWidth=1;for(let x=80;x<2000;x+=100){g.beginPath();g.moveTo(x,0);g.lineTo(x,512);g.stroke()}for(const y of [267,471])for(const x of [150,850,1670]){g.strokeStyle='#7b8e94';g.strokeRect(x,y,26,35)}const t=new T.CanvasTexture(c);t.colorSpace=T.SRGBColorSpace;t.anisotropy=aniso;return new T.MeshPhysicalMaterial({map:t,metalness:.2,roughness:.34,clearcoat:.7})}
 ...
 if(i===0){const end=trainer?9.5:jumbo?16.5:14.5,start=trainer?-5.5:jumbo?-14.5:-12.5;const rs=[];for(let j=0;j<=32;j++){const f=j/32,z=start+(end-start)*f;const nose=Math.min(1,Math.sqrt(Math.max(.001,(1-f)/.15)));rs.push([z,radius*nose,0,1])}mesh(surface(rs,64),livery(s.aircraft,s.livery),[0,0,0],content);const cap=mesh(new T.CircleGeometry(radius,48),aluminum,[0,0,start],content);cap.rotation.y=Math.PI;
 const lettering=new T.TextureLoader().load(s.livery.logo);lettering.colorSpace=T.SRGBColorSpace;lettering.anisotropy=aniso;const decalWidth=Math.min(trainer?5:10,(trainer?1:1.5)*s.livery.ratio);for(const side of [-1,1]){const decal=mesh(new T.PlaneGeometry(decalWidth,decalWidth/s.livery.ratio),new T.MeshStandardMaterial({map:lettering,transparent:true,depthWrite:false,roughness:.4}),[side*(radius+.035),.35,trainer?2:5],content);decal.rotation.y=side*Math.PI/2}}
 ...
 else if(i===6){const shape=new T.Shape();shape.moveTo(tail+3,0);shape.lineTo(tail-.5,trainer?4.5:8);shape.lineTo(tail-3,trainer?4.6:8.2);shape.lineTo(tail-4,0);const g=new T.ExtrudeGeometry(shape,{depth:.24,bevelEnabled:true,bevelSize:.05,bevelThickness:.05,bevelSegments:2,steps:1});g.rotateY(-Math.PI/2);mesh(g,navy,[.12,0,0],content);for(const side of [-1,1]){const mark=new T.TextureLoader().load(s.livery.logo);mark.colorSpace=T.SRGBColorSpace;const crops={delta:[0,0,.23,1],american:[.57,0,.43,1],united:[.82,0,.18,1],southwest:[.86,0,.14,1],qatar:[.72,0,.28,1],lufthansa:[0,0,.18,1],airfrance:[.87,0,.13,1],singapore:[.67,0,.33,1],british:[.4,.55,.6,.45]};const crop=crops[s.livery.id]||[0,0,1,1];mark.offset.set(crop[0],crop[1]);mark.repeat.set(crop[2],crop[3]);const w=trainer?1.5:2.2,h=Math.min(trainer?1.8:3,w/(s.livery.ratio*crop[2]/crop[3]));const badge=mesh(new T.PlaneGeometry(w,h),new T.MeshBasicMaterial({map:mark,transparent:true,depthWrite:false}),[side*.29,trainer?2.3:4.6,tail-1.3],content);badge.rotation.y=side*Math.PI/2}}
