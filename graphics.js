...
diffuseColor*=mix(sampleA,sampleB,blendTex);
 ...
 function build(s){if(!['twin','dc10','b747'].includes(s.aircraftFamily))throw new Error('Unregistered visual profile: '+s.aircraftFamily);disposeRoots();lastType=s.aircraft;lastParts=s.parts;const jumbo=s.aircraftFamily==='b747',trainer=s.aircraftFamily==='twin',radius=trainer?1.35:jumbo?2.15:1.8,tail=trainer?-10:jumbo?-18:-16;
 ...
 const lettering=textTexture('HORIZON',1024,160,'#213f55');for(const side of [-1,1]){const decal=mesh(new T.PlaneGeometry(trainer?5:9,.75),new T.MeshStandardMaterial({map:lettering,transparent:true,depthWrite:false,roughness:.4}),[side*(radius+.015),.75,trainer?2:5],content);decal.rotation.y=side*Math.PI/2}}
